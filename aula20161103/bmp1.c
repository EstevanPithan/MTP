...
{
    struct Pixel pixel;
    int WPX, HPX, BPP, PAL, i, j, AUX, NCP, NIC;
    FILE * imagem;
    unsigned char byte;
    unsigned short word;
    unsigned int dword;
    imagem = fopen("bmp1.bmp", "rb");
...
//  word	6-7	Informação reservada (não vai ter nada)
//  word	8-9	Informação reservada (não vai ter nada)
